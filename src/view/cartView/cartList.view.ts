...
export class CartListView {
  ...
  render(root: HTMLElement) {
    ...
    const cartList = Utils.create<HTMLElement>("cart-section__shopping shopping", "table");
  ...
  fillTable(root: HTMLElement) {
    const cartTable = Utils.create<HTMLElement>("shopping__table shopping-table", "table");
  ...
  addTableBody() {
    ...
    this.model.cartItems.forEach(product => {
      template += `<tr class="shopping-table__body-row table-item" id="cart-product-${product.id}">
                                        <td>
                                        ...
                                        <td>
                                            <span class="table-item__price table-item__price--default">${product.price} ₽</span>
                                        </td>
                                        <td>
                                            <span class="table-item__stock">${product.stock - product.stockUsed} шт.</span>
                                        </td>
                                        <td>
                                            <div class="table-item__quantity">
                                                <button class="button button--quantity" id="cart-qty-minus">-</button>
                                                <span class="table-item__input" id="input">${product.stockUsed}</span>
                                                <button class="button button--quantity" id="cart-qty-plus">+</button>
                                            </div>
                                        </td>

                                        <td>
                                            <span class="table-item__amount" id="amount">${product.price * product.stockUsed} ₽</span>
                                        </td>
                                    </tr>`;
      serialNumber++;
    });
